...
def account_selection(userID, gui = None):
    if gui:
        global account_name
...
def splash_screen(s, y, x):
    ...
    s.addstr(17, 53, "Start Cloud Manager", curses.color_pair(6))
...
def home_screen(nickname, logged_in=True):
    ...
    if logged_in == True:
        print JColor.FGRED + JColor.BOLDON + """   _____ _                 _   __  __                                   
  / ____| |               | | |  \/  |                                  
 | |    | | ___  _   _  __| | | \  / | __ _ _ __   __ _  __ _  ___ _ __ 
 | |    | |/ _ \| | | |/ _` | | |\/| |/ _` | '_ \ / _` |/ _` |/ _ \ '__|
 | |____| | (_) | |_| | (_| | | |  | | (_| | | | | (_| | (_| |  __/ |   
  \_____|_|\___/ \__,_|\__,_| |_|  |_|\__,_|_| |_|\__,_|\__, |\___|_|   
                                                         __/ |          
                                                        |___/           """ + JColor.ENDC + JColor.BOLDOFF + """
...
""" + JColor.ENDC + JColor.BOLDOFF + """
Hello """ + JColor.FGCYAN + JColor.BOLDON + nickname + JColor.ENDC + JColor.BOLDOFF + """! Welcome to Cloud Manager.
You are not logged in.
The date is """+ JColor.FGWHITE + formatted_date + JColor.ENDC +""" and the current time is """+ JColor.FGWHITE+formatted_time+JColor.ENDC #+"""     
# You can enter """+ JColor.FGORANGE + JColor.BOLDON +"""ls"""+ JColor.ENDC + JColor.BOLDOFF +""" to print your home directory or """+ JColor.FGORANGE + JColor.BOLDON +"""?"""+ JColor.ENDC + JColor.BOLDOFF +""" for more help."""                    

def dropbx_get_token(userID, gui = None):
    clearscreen()
    ...
    appkey = 'fxcey8m42nsukqe'
    appsecret = 'is17t4n5ijlrfsb'
    # try:
    ...
    # get user's nickname
    if gui:
        nickname = tkSimpleDialog.askstring('', "What do you want to name this dirve? ")
    elif gui == None:
        nickname = raw_input(
            JColor.BOLDON + JColor.FGCYAN + "What do you want to name this dirve? " + JColor.ENDC + JColor.BOLDOFF).strip()
        clearscreen()
    ...
    '''
    if gui:
        code = tkSimpleDialog.askstring('', "Enter the authorization code here? ")
    elif gui == None:
        code = raw_input(
            JColor.BOLDON + JColor.FGCYAN + "Enter the authorization code here: " + JColor.ENDC + JColor.BOLDOFF).strip()
    ...
    try:
        ...
        # raise
    try:
        addkey(nickname, encrypt_string(access_token), userID, 'dropbox')
    except:
...
# put our app to work!
def start_raspberry_cloud(s, access_token, userID, gui = None):
    ...
    class access_dropbox():
        ...
        if gui == None:
            ...
            client = dropbox.client.DropboxClient(decrypt_string(access_token))
        ...
        def print_dir(self, access_token, user_id, foldername):
        ...
        def check_filesize(self, current_dir, name_of_file, client):
            # list client folders
        ...
        def decrypt_file(self, key, in_filename, out_filename=None, chunksize=24 * 1024):
            ...
            with open('download/' + in_filename, 'rb') as infile:
                origsize = struct.unpack('<Q', infile.read(struct.calcsize('Q')))[0]
                iv = infile.read(16)
                decryptor = AES.new(key, AES.MODE_CBC, iv)

                with open('download/' +out_filename, 'wb') as outfile:
                    while True:
                        chunk = infile.read(chunksize)
                        if len(chunk) == 0:
                            break
        ...
        # for uploading files
        def new_upload(self, current_dir, path_to_file, upload_file_name, encrypt, client ):
            ...
            if (local_file_size >= 4194304):
                ...
                if(encrypt == "true"):
                    ...
                    self.encrypt_file("0000000000000000", path_to_file)
                    ...
                    metadata = client.metadata(current_dir + "/" + upload_file_name)
                    if metadata['content_hash'] != hash_file.main('download/' + path_to_file):
                        print 'upload fail'
                        return
                clearscreen()
                self.print_dir(access_token, user_id, current_dir)
               ...
               # print "[+]\tsuccessfully uploaded " + upload_file_name + " to " + current_dir
                ...
                try:
                    ...
                    if (encrypt == "true"):
                        ...
                        upload_file_size = self.check_filesize(current_dir, upload_file_name, client)
                        os.remove(path_to_file + '.enc')  # delete encrypted file
                    else:
                        print 'no encrypting'
                        ...
                        upload_file_size = self.check_filesize(current_dir, upload_file_name, client)
                        metadata = client.metadata(current_dir + "/" + upload_file_name)
                        if metadata['content_hash'] != hash_file.main(path_to_file):
                            print 'upload fail'
                            return
        ...
        def copy_file(self, client, old_file, new_file, current_dir):
            client.file_copy(current_dir + "/" + old_file, new_file)

        # function to backup files when they are being edited
        def backup_file(self, client, current_dir, file_to_edit):
        ...
        def download_file(self, current_dir, file_to_download, client, download_loc='download'):
            ...
            with client.get_file(current_dir + "/" + file_to_download) as f:
                out.write(f.read())
            out.close()
            metadata = client.metadata(current_dir + "/" + file_to_download)
            if metadata['content_hash'] != hash_file.main('download/' + file_to_download):
                print "download fail"
                return
            #print hash_file('download/' + file_to_download)
            #print get_checksum('download/' + file_to_download)
            print file_to_download[-4:]
            if(file_to_download[-4:] == ".enc"):
                print "start de"
                self.decrypt_file("0000000000000000", file_to_download)
                print file_to_download
                os.remove('download/' + file_to_download)  # delete encrypted file
            print "successfully downloaded " + file_to_download

        def upload_to_otherdp(self, current_dir, path_to_file, upload_file_name, client1, account_dict, account_name):
            self.download_file(current_dir, path_to_file.replace(".merge", ""), client1)
            type = account_dict[account_name]['type']
            if type == 'dropbox':
                self.new_upload(current_dir, 'download/'+path_to_file.replace(".merge", ""), upload_file_name, 'no', client2)
            elif type == 'google':
                gauth = GoogleAuth()
                gauth.credentials = OAuth2Credentials.from_json(decrypt_string(account_dict[account_name]['token']))
                drive = GoogleDrive(gauth)
                pycloud.upload('root', path_to_file, drive)

        def merge(self, gui = None):
            def read_dir(metadata, dir_dict, level, newlist):
                    ...
                    if type(value) is list:
                        for listitem in value:
                            ...
                            if listitem['is_dir'] == True:
                                metadata = self.client.metadata(listitem['path'])
            ...
            if gui == None:
                ...
                account_dict, account_name = account_selection(userID, True)

            #account_dict, account_name, account_type = account_selection(userID)
            client2 = dropbox.client.DropboxClient(decrypt_string(account_dict[account_name]['token']))
            current_dir = '/'
            # stick folder info in a variable
            src_folder_metadata = self.client.metadata(current_dir)
            des_folder_metadata = client2.metadata(current_dir)


            src_list = read_dir(src_folder_metadata, src_dir_dict, '1', src_list)
            des_list = read_dir(des_folder_metadata, des_dir_dict, '1', des_list)
            print src_list
            print src_list
            print des_list
            dul_files = set(src_list['1']) & set(des_list['1'])
            print dul_files
            print type(dul_files)
            final_list = []
            for dulfile in dul_files:
                print dulfile
                metadata1 = self.client.metadata(dulfile)
                metadata2 = client2.metadata(dulfile)
                if metadata1['content_hash'] == metadata2['content_hash']:
                    print dulfile + ' is same'
                    src_list.get('1').remove(dulfile)
                else:
                    print dulfile + ' is different'
                    src_list.get('1').remove(dulfile)
                    src_list.get('1').append(dulfile + '.merge')

            for file in src_list.get('1'):
                file_name = ntpath.basename(file)
                self.upload_to_otherdp(file.replace(file_name,""), file_name, file_name, self.client, account_dict, account_name)
            ...
            for key, value in folder_metadata.iteritems():
                if type(value) is list:
                    for listitem in value:
                        if type(listitem) is dict:
                            if listitem['is_dir'] == False:
                                upload_to_otherdp(current_dir, listitem['path'], ntpath.basename(listitem['path']), client, account_dict,account_name)
            '''

        def start(self, client, current_dir):
    ...
    """
                ...
                # go back to the root directory
                    ...
                    self.print_dir(access_token, user_id, "/")
                    current_dir = "/"
    ...
    """
                    ...
                    try:
                        ...
                        # backup the file
                        self.backup_file(client, current_dir, handle_input[1])
    ...
    """
                    ...
                    try:
                        ...
                        print JColor.FGBLUE + handle_input[0] + JColor.ENDC + " was successfully copied to " + JColor.FGBLUE + handle_input[1] + JColor.ENDC + " at " + JColor.FGWHITE + self.curr_time + JColor.ENDC
                    except Exception as ex:
                        error_img(str(ex))
                        # print ex
    ...
    """
                    ...
                    if (delete_prompt == 'Y') or (delete_prompt == 'y'):
                        try:
                            ...
                            self.print_dir(access_token, user_id, current_dir)
                            if list_pointer2 > 1:
                                print JColor.FGBLUE + list_to_string + JColor.ENDC + " were successfully deleted at " + JColor.FGWHITE + self.curr_time + JColor.ENDC
                            else:
                                print JColor.FGBLUE + list_to_string + JColor.ENDC + " was successfully deleted at " + JColor.FGWHITE + self.curr_time + JColor.ENDC
                        except Exception as ex:
    ...
    optional arguments:
      -h, --help  show this help message and exit                
    """
                elif 'upload' in handle_input and ('-e' not in handle_input):
                    handle_input = handle_input.replace("upload","")
                    handle_input = handle_input.replace("'","").strip()
                    handle_input = handle_input.split()
                    self.new_upload(current_dir, handle_input[0], handle_input[0], 'false', client)
                    ...
                    handle_input = handle_input.replace("'", "").strip()
                    handle_input = handle_input.split()
                    self.new_upload(current_dir, handle_input[0], handle_input[0], 'true', client)
    ...
    """
                    ...
                    while list_pointer <= len(handle_input) - 1:
                        try:
                            self.download_file(current_dir, handle_input[list_pointer], client)
                        except Exception as ex:
                            error_img(str(ex))
    ...
    """
                    ...
                    handle_input = handle_input.replace('"',"").strip()
                    try:
                        self.check_filesize(current_dir, handle_input, client)
                    except Exception as ex:
                        error_img(str(ex))
    ...
    optional arguments:
      -h, --help  show this help message and exit                    
    """
                elif 'otherdp' in handle_input:
                    handle_input = handle_input.replace("otherdp", "").strip()
                    handle_input = handle_input.replace("'", "").strip()
                    handle_input = handle_input.split()
                    account_dict, account_name, account_type = account_selection(userID)
                    self.upload_to_otherdp(current_dir, handle_input[0], handle_input[1], client, account_dict, account_name)
                    # upload_file(current_dir, handle_input[0], handle_input[1], client)
                elif handle_input == 'home':
                    clearscreen()
                    home_screen(self.nickname)
                elif handle_input == '?':
    ...
    if not gui:
        ac_dp = access_dropbox(decrypt_string(access_token), user_id)
        client = dropbox.client.DropboxClient(decrypt_string(access_token))
        ac_dp.start(client, '/')
    if gui:
        acc = access_dropbox(decrypt_string(access_token), user_id)
        print 'hello'
        return acc
